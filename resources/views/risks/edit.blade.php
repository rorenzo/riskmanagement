            ...
            <div class="row justify-content-center">
                <div class="col-md-8">
                    <div class="card shadow-sm">
                        <div class="card-header">{{ __('Modifica Dati Rischio') }}</div>
                        <div class="card-body">
                            <form method="POST" action="{{ route('risks.update', $risk->id) }}">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $risk->name) }}" required>
                                    @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>

                                <div class="mb-3">
                                    <label for="tipologia" class="form-label">{{ __('Tipologia') }}</label>
                                    <input type="text" class="form-control @error('tipologia') is-invalid @enderror" id="tipologia" name="tipologia" value="{{ old('tipologia', $risk->tipologia) }}">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('tipo_di_pericolo') is-invalid @enderror" id="tipo_di_pericolo" name="tipo_di_pericolo" value="{{ old('tipo_di_pericolo', $risk->tipo_di_pericolo) }}">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3">{{ old('description', $risk->description) }}</textarea>
                                    @error('description') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>

                                <div class="mb-3">
                                    <label for="misure_protettive" class="form-label">{{ __('Misure Protettive Implementate/Previste') }}</label>
                                    <textarea class="form-control @error('misure_protettive') is-invalid @enderror" id="misure_protettive" name="misure_protettive" rows="4">{{ old('misure_protettive', $risk->misure_protettive) }}</textarea>
                                    ...
                                    <div class="row">
                                        @forelse ($allPpes as $ppe)
                                            <div class="col-md-6 mb-2">
                                                ...
                                                <label class="form-check-label w-100" for="ppe_edit_{{ $ppe->id }}">{{ $ppe->name }}</label>
                                            </div>
                                        @empty
                                ...
                                <div class="d-flex justify-content-end mt-4">
                                    ...
                                    <button type="submit" class="btn btn-primary">{{ __('Aggiorna Rischio') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
