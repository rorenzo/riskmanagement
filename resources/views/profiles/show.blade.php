...
<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div>
                @can('update', $profile) {{-- Permesso generico per modifica profilo --}}
    ...
    <div class="py-5">
        <div class="container">
            ...
            {{-- Card Dati Anagrafici & Incarico/Mansione --}}
            <div class="card shadow-sm mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">{{ __('Dati Anagrafici e Ruoli Organizzativi') }}</h5>
                    @can('update', $profile) {{-- O permesso specifico 'manage roles_responsibilities profile' --}}
                        <a href="{{ route('profiles.roles_responsibilities.edit.form', $profile->id) }}" class="btn btn-outline-secondary btn-sm" title="{{ __('Modifica Incarico e Mansione S.P.P.') }}">
                            <i class="fas fa-user-tie me-1"></i> {{ __('Gestisci Incarico/Mansione') }}
                        </a>
                    @endcan
                </div>
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <p><strong>{{ __('Incarico Attuale') }}:</strong> {{ $profile->incarico_display_name ?? ($profile->incarico ?: __('N/D')) }}</p>
                        </div>
                        <div class="col-md-6">
                            <p><strong>{{ __('Mansione S.P.P. Attuale') }}:</strong> {{ $profile->mansione_spp_display_name ?? ($profile->mansione ?: __('N/D')) }}</p>
                        </div>
                    </div>
                    <hr>
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        @can('update', $profile) {{-- O permessi più specifici --}}
                ...
                <div class="card-body">
                    @if ($currentEmploymentPeriod)
                        ...
                        @if($currentSectionAssignment && $currentSectionAssignment->section) {{-- Aggiunto controllo per $currentSectionAssignment->section --}}
                        ...
                        @endif
                    @else
                        <p><strong>{{ __('Stato Impiego') }}:</strong> <span class="badge bg-secondary">{{ $profile->getDisplayStatusAttribute() }}</span></p> {{-- Usiamo l'accessor per lo stato dettagliato --}}
                    @endif
                </div>
            </div>
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">{{ __('Attività Svolte') }}</h5>
                     @can('update', $profile) {{-- Assumendo che modificare le attività sia parte dell'update del profilo --}}
                        {{-- Qui andrebbe un link a un form di modifica attività per il profilo, es: {{ route('profiles.activities.edit', $profile->id) }} --}}
                        {{-- <a href="#" class="btn btn-outline-secondary btn-sm"><i class="fas fa-tasks me-1"></i> Gestisci Attività</a> --}}
                    @endcan
                </div>
                <div class="card-body">
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @can ("update", $profile) {{-- o permesso specifico 'updatePpes profile' --}}
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm table-hover">
                            ...
                            <tbody>
                                @foreach($profile->sectionHistory as $sectionPivot) {{-- Rinominato per chiarezza --}}
                                    <tr>
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @can ("create health_check_record") {{-- O permesso più granulare se definito --}}
                                ...
                                <tr class="{{ $hsData['needs_attention'] ? 'table-danger' : ($hsData['has_record'] && !$hsData['is_expired'] && $hsData['expiration_date'] && \Carbon\Carbon::createFromFormat('d/m/Y', $hsData['expiration_date'])->isBetween(now(), now()->addMonths(2)) ? 'table-warning' : '') }}">
                                    ...
                                    <td class="text-center">
                                        @if(!$hsData['has_record'])
                                            ...
                                            @can("create health_check_record")
                                        ...
                                        @elseif($hsData['is_expired'])
                                            ...
                                            @if($hsData['record_id'])
                                                @can("update health_check_record") {{-- Assumendo permesso generico o specifico --}}
                                        ...
                                        @elseif($hsData['expiration_date'] && \Carbon\Carbon::createFromFormat('d/m/Y', $hsData['expiration_date'])->isBetween(now(), now()->addMonths(2)))
                                            ...
                                            @if($hsData['record_id']) @can("update health_check_record")
                                        ...
                                        @else
                                            ...
                                            @if($hsData['record_id'])
                                                @can("update health_check_record")
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @can ("create profile_safety_course") {{-- O permesso specifico se definito --}}
                                ...
                                <tr class="{{ $courseData['needs_attention'] ? 'table-danger' : ($courseData['is_attended'] && !$courseData['is_expired'] && $courseData['expiration_date'] && \Carbon\Carbon::createFromFormat('d/m/Y', $courseData['expiration_date'])->isBetween(now(), now()->addMonths(2)) ? 'table-warning' : '') }}">
                                    ...
                                    <td class="text-center">
                                        @if(!$courseData['is_attended'])
                                            ...
                                            @can ("create profile_safety_course")
                                        ...
                                        @else
                                            ...
                                            @if($courseData['attendance_pivot_id'])
                                                @can ("update profile_safety_course") {{-- O permesso specifico --}}
                        ...
                        <table class="table table-sm table-hover">
                            ...
                            <tbody>
                                ...
                                <tr class="{{ $courseData['is_expired'] ? 'table-danger' : ($courseData['expiration_date'] && \Carbon\Carbon::createFromFormat('d/m/Y', $courseData['expiration_date'])->isBetween(now(), now()->addMonths(2)) ? 'table-warning' : '') }}">
                                    ...
                                    <td class="text-center">
                                        @can ("update profile_safety_course")
                                            @if($courseData['attendance_pivot_id'])
