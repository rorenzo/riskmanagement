...
class AnagraficaController extends Controller
{
    ...
    public function __construct()
    {
        ...
        // Potrebbero essere raggruppati sotto 'update profile' o resi più specifici
        $this->middleware('permission:update profile' . $resourceName, [
            'only' => [
                'edit', 'update', // Modifica anagrafica base
                'editRolesAndResponsibilitiesForm', 'updateRolesAndResponsibilities',
                'createEmploymentPeriodForm', 'storeEmploymentPeriod',
                'editSectionAssignmentForm', 'updateSectionAssignment',
                'createTransferOutForm', 'storeTransferOut', // Logica di 'terminate employment profile'
                'editPpes', 'updatePpes',
                // Aggiungere qui altri metodi che modificano il profilo se necessario
            ]
        ]);
    ...
    public function data(Request $request)
    {
        try { // 
            $baseQuery = Profile::query()
                ->whereDoesntHave('employmentPeriods', function ($query) { //
                }) // 
            ...
            if ($request->has('order') && is_array($request->input('order')) && count($request->input('order')) > 0) { // 
                ...
                if (isset($columns[$orderColumnIndex]['data'])) { // 
                    ...
                    $sortMapping = [ // 
                        ...
                        'mansione_spp_display' => 'profiles.mansione', // 
            ...
            $data = $profiles->map(function ($profile) { // 
                ...
                return [ //
                    ...
                    'mansione_spp_display' => $mansioneSppDisplayName ?? __('N/D'), //
    ...
    public function create()
    {
        ...
        return view('profiles.create', compact('incarichiDisponibili', 'mansioniSppDisponibili'));
    }
    ...
    public function show(Profile $profile)
    {
        ...
        $profile->load([ //
            ...
            'safetyCourses', // Per caricare la relazione base
            'assignedPpes' //
        ]); // 
        $currentSectionAssignment = $profile->getCurrentSectionAssignmentWithPivot(); // Utilizza il metodo helper se definito nel model
            ...
            foreach ($tempRequiredCourses as $courseId => $data) { //
                ...
                if ($latestAttendance && isset($latestAttendance->pivot) && $latestAttendance->pivot->attended_date) { // 
                    ...
                    if ($data['course_object']->duration_years && $data['course_object']->duration_years > 0) { // 
                        $expirationDateCarbon = $attendedDateCarbon->copy()->addYears($data['course_object']->duration_years); $expirationDate = $expirationDateCarbon->format('d/m/Y'); $isExpired = $expirationDateCarbon->isPast(); // 
                    } // 
                }
            ...
            foreach ($profile->safetyCourses as $attendedCourse) { //
                if (!in_array($attendedCourse->id, $activityBasedRequiredCourseIds)) { //
                    ...
                    if ($attendedCourse->duration_years && $attendedCourse->duration_years > 0) { //
                        $expirationDateCarbon = $attendedDateCarbon->copy()->addYears($attendedCourse->duration_years); $expirationDate = $expirationDateCarbon->format('d/m/Y'); // 
            ...
            foreach ($tempRequiredHS as $hsId => $data) { //
                ...
                if ($latestCheckUpRecord) { // 
                    ...
                    if ($latestCheckUpRecord->expiration_date) { // 
                        $expirationDateCarbon = Carbon::parse($latestCheckUpRecord->expiration_date); $expirationDate = $expirationDateCarbon->format('d/m/Y'); $isExpired = $expirationDateCarbon->isPast(); // 
                    } // 
                }
            ...
            foreach ($profile->healthCheckRecords as $record) { //
                if (!in_array($record->health_surveillance_id, $activityBasedRequiredHSIds)) { //
                    ...
                    if ($record->expiration_date) { //
                        $expirationDateCarbon = Carbon::parse($record->expiration_date); $expirationDate = $expirationDateCarbon->format('d/m/Y'); $isExpired = $expirationDateCarbon->isPast(); // 
                    } // 
        ...
        return view('profiles.show', compact( //
            'profile', 'currentSectionAssignment', 'currentEmploymentPeriod', 'incarichiDisponibili', 'mansioniSppDisponibili', //
    ...
    public function edit(Profile $profile)
    {
        ...
        $incarichiDisponibili = Profile::INCARICHI_DISPONIBILI;
        $mansioniSppDisponibili = Profile::MANSIONI_SPP_DISPONIBILI;
        return view('profiles.edit', compact('profile', 'incarichiDisponibili', 'mansioniSppDisponibili'));
    }

    public function update(Request $request, Profile $profile)
    {
        $this->authorize('update', $profile);
        // Questa validazione ora è solo per i dati anagrafici base
        $validatedData = $request->validate([
            'grado' => 'nullable|string|max:50', //
            'nome' => 'required|string|max:255', //
            'cognome' => 'required|string|max:255', // 
            ...
            // Incarico e mansione sono gestiti da updateRolesAndResponsibilities
            'residenza_via' => 'nullable|string|max:255', //
            'residenza_citta' => 'nullable|string|max:255', //
            'residenza_provincia' => 'nullable|string|max:2', // 
            'residenza_cap' => 'nullable|string|max:5', //
    ...
    public function editRolesAndResponsibilitiesForm(Profile $profile)
    {
        $this->authorize('update', $profile);
        $incarichiDisponibili = Profile::INCARICHI_DISPONIBILI;
        $mansioniSppDisponibili = Profile::MANSIONI_SPP_DISPONIBILI;
        return view('profiles.form_roles_responsibilities', compact('profile', 'incarichiDisponibili', 'mansioniSppDisponibili'));
    }

    public function updateRolesAndResponsibilities(Request $request, Profile $profile)
    {
        $this->authorize('update', $profile);
        $incarichiRule = Rule::in(array_keys(Profile::INCARICHI_DISPONIBILI));
        $mansioniSppRule = Rule::in(array_keys(Profile::MANSIONI_SPP_DISPONIBILI));
        ...
        try {
            ...
            $profile->update($validatedData);
            DB::commit();
            return redirect()->route('profiles.show', $profile->id)->with('success', __('Incarico e Mansione S.P.P. aggiornati con successo.'));
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Errore aggiornamento incarico/mansione profilo ID ' . $profile->id . ': ' . $e->getMessage());
            return back()->withInput()->with('error', __('Errore durante l\'aggiornamento di Incarico/Mansione S.P.P.:') . ' ' . $e->getMessage());
        }
    }
    ...
    public function createEmploymentPeriodForm(Profile $profile)
    {
        ...
        return view('profiles.form_employment_period', compact('profile', 'sections', 'tipiIngresso')); // 
    }
    ...
    public function storeEmploymentPeriod(Request $request, Profile $profile)
    {
        ...
        try {
            ...
            // Termina assegnazioni attive precedenti
 ...
 'Assegnazione manuale del ' . Carbon::now()->format('d/m/Y'), // 
    ...
    public function destroy(Profile $profile)
    {
        ...
        try {
            ...
            $currentSectionAssignment = $profile->getCurrentSectionAssignmentWithPivot(); // Utilizza il metodo che ritorna anche i dati pivot
            if ($currentSectionAssignment) { // 
            ...
            ProfileSafetyCourse::where('profile_id', $profile->id)->delete(); //  Soft delete pivot records
            $profile->healthCheckRecords()->delete(); //  Soft delete related records
    ...
    {
        $this->authorize('viewAny archived_profiles', Profile::class); // O permesso specifico 'viewAny archived_profiles'
        $user = Auth::user(); // 
        $userPermissions = [ //
            'can_view_profile' => $user->can('view profile'), // 
            'can_edit_profile' => $user->can('update profile'), //
            'can_delete_profile' => $user->can('delete profile'), //
            'can_restore_profile' => $user->can('restore profile'), //
            'can_create_employment' => $user->can('update profile'), // Permesso per riattivare con nuovo impiego
        ]; // 
    ...
    {
        $this->authorize('viewAny archived_profiles', Profile::class); // O 'viewAny archived_profiles'
        try { // 
            $baseQuery = Profile::query()
    ->withTrashed() // Include sempre i soft-deleted
    ->where(function ($query) {
        $query->whereNotNull('deleted_at') // 1. Profili soft-deleted
                ->orWhere(function ($queryNonDeletati) { // 2. Oppure profili non soft-deleted che sono inattivi
                    $queryNonDeletati->whereNull('deleted_at')
                                     ->where(function ($qInactive) {
                                         // Sono inattivi se:
                                         $qInactive->whereDoesntHave('employmentPeriods') // a) Non hanno MAI avuto periodi di impiego
                                                   ->orWhere(function ($qAllPeriodsEnded) { // b) OPPURE tutti i loro periodi di impiego sono terminati
                                                       $qAllPeriodsEnded->whereHas('employmentPeriods') // Hanno avuto periodi
                                                                        ->whereDoesntHave('employmentPeriods', function ($epQuery) {
                                                                            $epQuery->whereNull('data_fine_periodo'); // Ma nessuno è attivo
                                                                        });
                                                   });
                                     });
                });
    ...
    public function restore($id)
    {
        ...
        try {
            DB::beginTransaction(); // 
            $profile->restore(); // 
            // L'utente dovrà creare un nuovo periodo di impiego da profiles.show
            DB::commit(); // 
            return redirect()->route('profiles.show', $profile->id)
    ...
    public function forceDelete($id)
    {
        ...
        try {
            DB::beginTransaction(); // 
            // Detach/delete relazioni (molte tabelle pivot hanno onDelete('cascade'))
